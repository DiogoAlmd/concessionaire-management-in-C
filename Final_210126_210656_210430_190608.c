...
void gravaReservaM(loja *p, char *str, int pos);
void gravaReservaC(montadora *p, char *str, int pos);
//reservar carros
...
void recebeMontadora(montadora *p, int qtde){
	...
	p->status.sigla = 'L'; //status inicializado com 'L'
		...
		if(p->reserved<=3){
			...
			if(pos==-1){
				...
				if(p2->status.sigla == 'R'){
					...
					p2->status.sigla = 'R';  //reserva interna com 'R';
				...
				//	printf("\nTabela[%i]: %c - %i\n", i, p->tabela[i].sigla, p->tabela[i].reservado.regcarro);
					...
					printf("\nDeseja realmente reservar este carro? <S/N>");
					scanf("%c", &op2);
					fflush(stdin);
					op2 = toupper(op2);
			...
			if(p2->status.sigla == 'R')
			{
				...
				switch(op2)
				{
					case 1: printf("\nInicio do processo de Venda.\n");
							...
							for(i=0;i<3;i++){
								if(p->tabela[i].reservado.regcarro == num_reg){
									//printf("\nTabelas depois da venda");
									printf("\nTabela[0]: %c - %i", p->tabela[0].sigla, p->tabela[0].reservado.regcarro);
		        					printf("\nTabela[1]: %c - %i", p->tabela[1].sigla, p->tabela[1].reservado.regcarro);
		        					printf("\nTabela[2]: %c - %i\n", p->tabela[2].sigla, p->tabela[2].reservado.regcarro);
									//scanf("%i", &i);
					...
					case 2: //printf("\nInicio do processo de libera��o.\n");
							...
							printf("\nTabelas da empresa antes de serem liberadas:");
							printf("\nTabela[0]: %c - %i", p->tabela[0].sigla, p->tabela[0].reservado.regcarro);
        					printf("\nTabela[1]: %c - %i", p->tabela[1].sigla, p->tabela[1].reservado.regcarro);
        					printf("\nTabela[2]: %c - %i\n", p->tabela[2].sigla, p->tabela[2].reservado.regcarro);
							//scanf("%i", &i);
							...
							for(i=0;i<3;i++){
								if(p->tabela[i].reservado.regcarro == num_reg){
									...
									printf("\nTabelas da empresa depois de serem liberadas:");
									printf("\nTabela[0]: %c - %i", p->tabela[0].sigla, p->tabela[0].reservado.regcarro);
		        					printf("\nTabela[1]: %c - %i", p->tabela[1].sigla, p->tabela[1].reservado.regcarro);
		        					printf("\nTabela[2]: %c - %i\n", p->tabela[2].sigla, p->tabela[2].reservado.regcarro);
								}
